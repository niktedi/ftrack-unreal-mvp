...
``session``, ``context``, ``publish.publisher``, ``publish.image_sequence``,
``asset_manager.tree_model``, ``asset_manager.details``,
``asset_manager.updates``, ``asset_manager.stamps``
    Pure Python. ftrack_api only, never ``import unreal``.

``unreal_env``, ``menu``, ``ui_bridge``, ``publish.camera_fbx``,
``publish.thumbnail``, ``publish.mrq_render``, ``asset_manager.importer``,
...
#: Short code of the ftrack asset type rendered Level Sequences are published
